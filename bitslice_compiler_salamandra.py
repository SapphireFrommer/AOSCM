...
def SCM_design(params, scm):
    ...
    #scm['TOP'].uniq()
    for module in sorted(scm['bitslice'].get_subcomponents_dict().keys()):
        print(module +'\t->\t'+ str(scm['bitslice'].get_subcomponents_dict()[module]))
...
def SCM_design_components_instances(params, scm):
    ...
    SCM_design_components_instances_bitslice(params, scm, 'bitslice')
...
def SCM_design_components_instances_read_mux(params, scm):
    ...
    ADDR_WIDTH = params['ADDR_WIDTH']
    read_mux = scm['read_mux']
    NAND2 = sc['NAND2_X1']['component']
    NOR2 = sc['NOR2_X1']['component']
...
def write_verilog_file(params,scm):
    ...
    header_comment_text = []
    module_scm_text = scm['bitslice'].write_verilog()
    ...
    for module in scm['bitslice'].get_subcomponents_recursive(inclusive=True):
...
def write_tcl(params,scm):
    ...
    for module in [scm['bitslice']]:
        ...
        place_instance_list,create_group_list,add_inst_to_group_list = module.write_tcl_placement_commands(params['TCL_tool']) 
        for line in place_instance_list:	# standatd cells placement
            tcl_file.write(line + '\n')     
        tcl_file.close()
